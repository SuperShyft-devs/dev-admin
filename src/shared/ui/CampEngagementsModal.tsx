...
interface CampEngagementsModalProps {
  open: boolean;
  onClose: () => void;
  campNo: number;
  campName?: string;
...
export function CampEngagementsModal({
  open,
  onClose,
  campNo,
  campName,
  orgName,
}: CampEngagementsModalProps) {
  ...
  const fetchEngagements = useCallback(async () => {
    ...
    try {
      ...
      while (true) {
        const res = await engagementsApi.list({ page, limit: pageSize, camp_no: campNo });
  ...
  const title = campName
    ? `Engagements — ${campName}`
    : `Engagements — Camp #${campNo}`;

  return (
    <Modal open={open} onClose={onClose} title={title} maxWidthClassName="max-w-4xl">
      {orgName && (
        <p className="text-sm text-zinc-500 mb-4 -mt-2">Organisation: {orgName}</p>
      )}
        ...
        <div className="py-12 flex flex-col items-center gap-3 text-zinc-400">
          ...
          <p className="text-sm">
            {engagements.length === 0
              ? "No engagements found for this camp."
